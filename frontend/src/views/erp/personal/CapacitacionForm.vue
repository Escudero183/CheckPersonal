<template>
  <b-sidebar
    id="manage-capacitacion-sidebar"
    :visible="isToggleCapaSidebar"
    bg-variant="white"
  ...
  >
    <template #default="{ hide }">
      ...
      <div class="d-flex justify-content-between align-items-center content-sidebar-header px-2 py-1">
        <h5 class="mb-0">
          Gestionar Capacitaciones: {{ dataEdit.nombres }}
        </h5>
      ...
      >
        ...
        >
          ...
          <b-row>
            ...
            >
              <b-form-group
                label="Centro que Certifica"
                label-for="centroCertifica"
              >
                <b-form-input
                  id="centroCertifica"
                  v-model="formData.centroCertifica"
                />
              </b-form-group>
            </b-col>
            <b-col
            ...
            >
              <b-form-group
                label="Tema Capacitación"
                label-for="capacitacion"
              >
                <b-form-input
                  id="capacitacion"
                  v-model="formData.capacitacion"
                />
              </b-form-group>
            </b-col>
            <b-col
            ...
            >
              ...
              >
                <b-form-input
                  id="cantidadHoras"
                  v-model="formData.cantidadHoras"
                  type="number"
                />
              </b-form-group>
            </b-col>
        ...
        >
          ...
          >
            ...
            <span class="align-middle ml-25">Lista de Capacitaciones</span>
            <b-button
              v-ripple.400="'rgba(255, 255, 255, 0.15)'"
              variant="primary"
              class="float-right"
          ...
          >
            ...
            <template #cell(capacitacion)="data">
              <b-media vertical-align="center">
                <template #aside>
                  <b-avatar
                    :id="`capa-registro-${data.item.idCapacitacion}`"
                    size="32"
                ...
                <span class="font-weight-bold d-block text-nowrap">
                  {{ data.item.capacitacion }}
                </span>
                <small class="text-muted d-block">Emitido el {{ data.item.fechaEmision }}    Horas lectivas {{ data.item.cantidadHoras }}</small>
              </b-media>
            </template>
            <!-- Column: Actions -->
...
export default {
  ...
  model: {
    prop: 'isToggleCapaSidebar',
    event: 'update:is-toggle-capa-sidebar',
  },
  props: {
    isToggleCapaSidebar: {
      type: Boolean,
  ...
  setup(props, context) {
    ...
    const fields = ref([
      { key: 'idCapacitacion', sortable: true, thClass: 'd-none', tdClass: 'd-none' },
      { key: 'capacitacion', sortable: true },
    ...
    const getRegistros = async () => {
      ...
      await store.dispatch('personal/CAPACITACION_FIND_ALL', {
    ...
    const getDelete = data => {
      Vue.swal({
        ...
        if (result.value) {
          await store
            .dispatch('personal/CAPACITACION_DELETE', {
              id: data.item.idCapacitacion,
            })
            .then(() => {
    ...
    const saveForm = async () => {
      isBusy.value = true
      // Validar datos antes
      let service = 'personal/CAPACITACION_CREATE'
      if (formType.value === 'edit') {
        service = 'personal/CAPACITACION_UPDATE'
      }
      ...
      await store.dispatch(service, formData.value)
        .then(async response => {
          ...
          // emit('update:is-toggle-capa-sidebar', false)
        })
        .catch(error => {
          context.emit('error-data', error)
          context.emit('update:is-toggle-capa-sidebar', false)
        })
      isBusy.value = false
      isNewRegister.value = false
    }
    ...
    const toggleSidebar = async val => {
      context.emit('update:is-toggle-capa-sidebar', val)
      await getRegistros()
    }
